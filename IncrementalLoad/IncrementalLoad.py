...
try:
    ...
    last_id_df = spark.read.text("s3://22septbuck/output/last_loaded_id.txt")
...
df = spark.read.format("csv") \
    .option("header", "true") \
    .load("s3://22septbuck/details.csv") \
...
df2.write.format("csv") \
    .option("header", "true") \
    .mode("append") \
    .save("s3://22septbuck/output/Finaloutput/")
...
if not df.isEmpty():
    ...
    id_df.coalesce(1).write.mode("overwrite").text("s3://22septbuck/output/last_loaded_id.txt")
